...
import { ITEM_DATABASE, RARITY_CONFIG, calculateInventorySlots, MAX_INVENTORY_SLOTS, EXPANDED_INVENTORY_SLOTS } from '@/lib/loot';
...
export default function Inventory() {
  ...
  const maxSlots = player.has_expanded_inventory ? EXPANDED_INVENTORY_SLOTS : MAX_INVENTORY_SLOTS;
  const currentSlots = calculateInventorySlots(inventory);
  const slotsPercent = (currentSlots / maxSlots) * 100;
  const slotsColor = slotsPercent > 90 ? '#dc2626' : slotsPercent > 70 ? '#f59e0b' : '#39ff14';
  ...
  const craftGrenade = async () => {
    const scrap = inventory.find(i => i.item_id === 'scrap_metal');
    const powder = inventory.find(i => i.item_id === 'gunpowder');
    
    if (!scrap || !powder || scrap.quantity < 2 || powder.quantity < 2) {
      addNotification('Faltam materiais! Precisa de 2 Sucatas de Metal e 2 Pólvoras.', 'danger');
      return;
    }
    
    const consumeItem = async (item: typeof scrap, qty: number) => {
    ...
    await consumeItem(scrap, 2);
    await consumeItem(powder, 2);
    
    const existingGrenade = inventory.find(i => i.item_id === 'grenade');
    if (existingGrenade) {
      updateInventoryItem(existingGrenade.id, { quantity: existingGrenade.quantity + 1 });
      await supabase.from('inventory').update({ quantity: existingGrenade.quantity + 1 }).eq('id', existingGrenade.id);
    } else {
      const def = ITEM_DATABASE['grenade'];
      const newItem = {
          id: crypto.randomUUID(),
          player_id: player.id,
          item_type: def.item_type,
    ...
    addNotification('Você fabricou 1x Granada de Fragmentação!', 'success');
  };

  const craftRecipes = [
    {
       id: 'ammo',
       title: 'MUNIÇÃO 9MM (x15)',
       canCraft: (inventory.find(i => i.item_id === 'gunpowder')?.quantity || 0) >= 1 && (inventory.find(i => i.item_id === 'scrap_metal')?.quantity || 0) >= 1,
       action: craftAmmo,
       ingredients: [
         { label: 'Pólvora Escura (1x)', ok: (inventory.find(i => i.item_id === 'gunpowder')?.quantity || 0) >= 1 },
         { label: 'Sucata de Metal (1x)', ok: (inventory.find(i => i.item_id === 'scrap_metal')?.quantity || 0) >= 1 }
       ]
    },
    {
       id: 'grenade',
       title: 'GRANADA DE FRAGMENTAÇÃO (x1)',
       canCraft: (inventory.find(i => i.item_id === 'gunpowder')?.quantity || 0) >= 2 && (inventory.find(i => i.item_id === 'scrap_metal')?.quantity || 0) >= 2,
       action: craftGrenade,
       ingredients: [
         { label: 'Pólvora Escura (2x)', ok: (inventory.find(i => i.item_id === 'gunpowder')?.quantity || 0) >= 2 },
         { label: 'Sucata de Metal (2x)', ok: (inventory.find(i => i.item_id === 'scrap_metal')?.quantity || 0) >= 2 }
       ]
    }
  ].sort((a, b) => (b.canCraft ? 1 : 0) - (a.canCraft ? 1 : 0));
      ...
      >
          ...
          <div className="flex items-center gap-4">
            ...
            <div style={{ minWidth: 120 }}>
              <div className="flex justify-between mb-1" style={{ fontSize: 9, color: '#666', fontFamily: "'Share Tech Mono', monospace" }}>
                <span>ESPAÇOS</span>
                <span style={{ color: slotsColor }}>{currentSlots}/{maxSlots}</span>
              </div>
              <div className="bar-container" style={{ height: 6 }}>
                <div style={{ width: `${slotsPercent}%`, height: '100%', background: slotsColor, transition: 'all 0.2s' }} />
              </div>
            </div>
            {!player.has_expanded_inventory && (
              <button
                className="btn-retro btn-retro-green"
                style={{ padding: '4px 8px', fontSize: 8 }}
                onClick={() => {
                  alert("Para expandir para 1000 espaços, faça um PIX de R$ 1,99 para: 00020101021126830014br.gov.bcb.pix01366c83942f-54fb-4f40-aabb-e44ca2f84f1a0221Expandir Inventário52040000530398654041.995802BR5925EUCLIDES RUFO SILVA DO NA6008BRASILIA62170513SXQRDeadWorld6304282B\nPIX COPIADO!");
                  navigator.clipboard.writeText("00020101021126830014br.gov.bcb.pix01366c83942f-54fb-4f40-aabb-e44ca2f84f1a0221Expandir Inventário52040000530398654041.995802BR5925EUCLIDES RUFO SILVA DO NA6008BRASILIA62170513SXQRDeadWorld6304282B");
                }}
              >
                + ESPAÇO
              </button>
          ...
          {activeTab === 'craft' ? (
            <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
              ...
              {craftRecipes.map(recipe => (
                <div key={recipe.id} style={{ background: 'rgba(20,20,20,0.8)', padding: 16, border: recipe.canCraft ? '1px solid #39ff14' : '1px solid #333', marginBottom: 12 }}>
                  <h3 className="pixel-font" style={{ color: recipe.canCraft ? '#39ff14' : '#dc2626', fontSize: 10, marginBottom: 12 }}>FABRICAR {recipe.title}</h3>
                  <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 16, fontSize: 10, fontFamily: "'Share Tech Mono', monospace" }}>
                    <div style={{ flex: 1, color: recipe.ingredients[0].ok ? '#39ff14' : '#555' }}>
                      {recipe.ingredients[0].label} {recipe.ingredients[0].ok ? '✓' : '✗'}
                    </div>
                    <strong style={{ color: '#555' }}>+</strong>
                    <div style={{ flex: 1, color: recipe.ingredients[1].ok ? '#39ff14' : '#555' }}>
                      {recipe.ingredients[1].label} {recipe.ingredients[1].ok ? '✓' : '✗'}
                    </div>
                  </div>
                  <button
                    className="btn-retro btn-retro-yellow"
                    onClick={recipe.action}
                    style={{ width: '100%', padding: '12px', opacity: recipe.canCraft ? 1 : 0.4 }}
                  >
                    FABRICAR AGORA
                  </button>
                </div>
        ...
        <div className="flex gap-3 px-4 py-2 border-t flex-wrap" style={{ borderColor: '#222' }}>
          ...
          <div style={{ marginLeft: 'auto', fontSize: 9, color: '#555', fontFamily: "'Share Tech Mono', monospace" }}>
            {inventory.length} ITENS · {currentSlots}/{maxSlots} ESPAÇOS
          </div>
        </div>
      </div>
    </div>
